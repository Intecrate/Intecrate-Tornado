...
def admin_test():
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~
    r = requests.get(TestHandler.make_url("/admin/challenge/list"), headers=headers)
    ...
    ), "challenge id from /admin/challenge/list is unbound"
    ...
    r = requests.post(
        ...
        json=datamodel.ChallengeRequest(challengeId=challenge.id).model_dump(
            by_alias=True
        ),
        headers=headers,
    )
    ...
    r = requests.delete(
        ...
        json=datamodel.ChallengeRequest(challengeId=new_challenge.id).model_dump(
            by_alias=True
        ),
        headers=headers,
    )
    ...
    if r.status_code != 502:
        raise TestFailure(
            f"Expected code 502 for fetching deleted challenge; got {r.status_code}"
        )
    ...
    r = requests.post(
        ...
        json=datamodel.StepRequest(stepId=challenge.steps[0]).model_dump(by_alias=True),
        headers=headers,
    )
    TestHandler.raise_for_status(r)
    ...
    routes = TestHandler.cloud_manager.api.get_map()
    admin_endpoints = [(end, obj) for end, obj in routes if end.startswith("/admin")]

    print("Checking auth on the following:")
    for e, _ in admin_endpoints:
        print(f" - {e}")

    # Check that admin api key is required for all endpoints
    for endpoint, handler in admin_endpoints:
        if getattr(handler, "TEST_IGNORE", False):
            print(f"info: skipping auth check on {endpoint}")
            continue

        if "post" in handler.__dict__:
            r = requests.post(TestHandler.make_url(endpoint))
        elif "get" in handler.__dict__:
            r = requests.get(TestHandler.make_url(endpoint))
        elif "delete" in handler.__dict__:
            r = requests.delete(TestHandler.make_url(endpoint))
        else:
            raise TestFailure(
                f"{endpoint} handler {handler.__name__} has no function to handle"
            )

        if r.status_code != 403:
            raise TestFailure(
                f"{endpoint} should have raised 403 when no auth, got {r.status_code}"
            )
        else:
